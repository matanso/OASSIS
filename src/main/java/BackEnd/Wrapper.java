...
public class Wrapper 
{
    ...
    /***
     * Query string is key, the algo that goes with it is value
     */
        ...
        }finally
 {
     ...
     * @param ontology
     * @return MyResult.validAnswer = 0 : success
    ...
    /***
     * input: query is in string format, suppToMakeSig - support threshold to mark assignment as significant. usersToMakeSig - 
     * number of users that make assignment significant.
     * @param String query, int suppToMakeSig, int usersToMakeSig
     * @return  Init's query (making it ready to be asnwered) and returns: MyResult class where its validAnswer field means:
     *          0 = success 
     ...
     *          4 = query is null
     */
    public static MyResult submitQuery(String query, int suppToMakeSig, int usersToMakeSig)
    {
        ...
        {
            return initQuery(query, suppToMakeSig, usersToMakeSig);
        }
    }
     ...
     * number of users that make assignment significant.
     * @param String query, int suppToMakeSig, int usersToMakeSig
     * @return MyResult class where its validAnswer field means:
     *         0 = success 
     *         1 = couldn't connect to repository.
     *         2 = query is malformed.
     *         3 = query couldn't be answered.
     */
    private static MyResult initQuery(String query, int suppToMakeSig, int usersToMakeSig)
    {
        /*input: query is in string format
        ...
        TraversalModule algo = new NaiveAlgorithm(queryResult, suppToMakeSig, usersToMakeSig);
    ...
    /***
     * input: bindingSet is the question that the user answered, queryID and userID same as the next function.
     * Support is the answer given by userID.
     * @param bindingSet
     * @param query
     * @param userID
     * @param support
    ...
    private static Map<String, TraversalModule> getAlgoDic()
 {
        ...
        try
 {
            Result result = NaiveAlgorithm.graphDB.execute( "match (n 
 {name: 'algoDic'}) return n" );
